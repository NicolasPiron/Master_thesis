...
def find_matrices(mat_dir):
    
    matrices = glob.glob(mat_dir + '/*-plv-*csv')
    mat_dict = {}
    for mat in matrices:
        mat_name = mat.split('/')[-1]
        mat_dict[mat_name] = pd.read_csv(mat, index_col=0)
...
def main2():
    ...
    input_dir, _ = get_paths()
    input_dir = os.path.join(input_dir, 'all_subj', 'resting-state', 'connectivity', 'static')
    ...
    for dir in mat_dirs:
        print(dir)
        mat_dict = find_matrices(os.path.join(input_dir, dir, 'conn_data'))
        for mat_name, matrix in mat_dict.items():
            print('lol')
            matrix = reorder_matrix(matrix, channel_order)
            if not os.path.exists(os.path.join(input_dir, dir, 'conn_data', 'reorg')):
                os.makedirs(os.path.join(input_dir, dir, 'conn_data', 'reorg'))
            plot_and_save_reorg_matrix(matrix, mat_name, os.path.join(input_dir, dir, 'conn_data', 'reorg'))
